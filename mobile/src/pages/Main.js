...
function Main({ navigation }) {
  ...
  useEffect(() => {
    async function loadInitialPsition() {
  ...
  function handleRegonChanged(region) {
  ...
  return (
    <>
      <MapView onRegionChangeComplete={handleRegonChanged} initialRegion={currentRegion} style={styles.map}>
        {devs.map(dev => (
          <Marker key={dev._id} coordinate={{ latitude: dev.location.coordinates[1], longitude: dev.location.coordinates[0] }}>
            ...
            <View style={[styles.heart, styles.heartShape]}>
